...
const STATUS_CONFIG = {
  ...
  completed: { label: 'Completed', color: 'bg-emerald-600 text-white', icon: IndianRupee },
                    ...
                    >
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                        ...
                        <div className="space-y-4">
                          <h4 className="text-[10px] font-black uppercase tracking-widest text-neutral-400">AI Verification Result</h4>
                          <div className="space-y-2">
                            ...
                            <div className="flex items-center justify-between p-3 bg-white rounded-2xl border border-neutral-100">
                              ...
                              <div className="flex items-center gap-2">
                                <span className="text-[10px] font-black text-neutral-400">Score: {claim.fraudScore}</span>
                                {claim.fraudScore < 0.5 ? <CheckCircle2 size={16} className="text-emerald-500" /> : <ShieldAlert size={16} className="text-amber-500" />}
                              </div>
                            </div>
                            <div className="flex items-center justify-between p-3 bg-white rounded-2xl border border-neutral-100">
                          ...
                          {claim.aiExplanation && (
                            <div className="p-4 bg-emerald-50 border border-emerald-100 rounded-2xl">
                              <p className="text-[10px] font-black text-emerald-700 uppercase tracking-widest mb-1">AI Decision Reasoning</p>
                              <p className="text-xs text-emerald-600 font-medium leading-relaxed">{claim.aiExplanation}</p>
                            </div>
                          )}
                        </div>
                        ...
                        {/* Payout Calculation */}
                        <div className="space-y-4">
                          <h4 className="text-[10px] font-black uppercase tracking-widest text-neutral-400">Transparency Panel</h4>
                          <div className="bg-white p-4 rounded-2xl border border-neutral-100 space-y-3">
                            <div className="flex justify-between text-xs">
                              ...
                              <span className="font-black text-neutral-900 capitalize">{claim.triggerEvent.split('_').join(' ')}</span>
                            </div>
                            <div className="flex justify-between text-xs">
                              <span className="text-neutral-500 font-medium">Lost Hours</span>
                              <span className="font-black text-neutral-900">{claim.lostHours || 0} Hours</span>
                            </div>
                            <div className="flex justify-between text-xs">
                              <span className="text-neutral-500 font-medium">Calculation</span>
                              <span className="font-black text-neutral-900 text-[10px]">{claim.compensationCalculation || 'Standard Formula'}</span>
                            </div>
                        ...
                        {/* Payment Status */}
                        <div className="space-y-4">
                          <h4 className="text-[10px] font-black uppercase tracking-widest text-neutral-400">Payment Status</h4>
                          <div className="bg-white p-6 rounded-2xl border border-neutral-100 flex flex-col items-center justify-center text-center">
                            <div className={`w-12 h-12 rounded-full flex items-center justify-center mb-3 ${claim.payoutStatus === 'completed' ? 'bg-emerald-100 text-emerald-600' : 'bg-blue-100 text-blue-600'}`}>
                              {claim.payoutStatus === 'completed' ? <CheckCircle2 size={24} /> : <Clock size={24} className="animate-spin" />}
                            </div>
                            <p className="text-sm font-black text-neutral-900 capitalize">{claim.payoutStatus || 'Processing'}</p>
                            <p className="text-[10px] text-neutral-400 font-medium mt-1">
                              {claim.payoutStatus === 'completed' 
                                ? `₹${claim.amount} credited to your wallet.` 
                                : 'Simulated payout in progress...'}
                            </p>
                          </div>
                        </div>
