...
export const Profile: React.FC = () => {
  ...
  const handleSwitchToAdmin = async () => {
    ...
    try {
      ...
      toast.success("Admin role activated successfully!");
      setSuccess("Your role has been updated to Admin. Please refresh if changes don't appear.");
    } catch (err) {
      console.error("Error switching to admin:", err);
      setError("Failed to update role. Please contact support.");
    } finally {
      setLoading(false);
    }
  };

  const isAuthorizedAdmin = profile.email && AUTHORIZED_ADMINS.map(a => a.toLowerCase().trim()).includes(profile.email.toLowerCase().trim());

  const infoItems = [
    { icon: Mail, label: 'Email', value: profile.email || 'Not Set' },
    { icon: Phone, label: 'Phone', value: profile.phoneNumber || 'Not Set' },
    { icon: Shield, label: 'Category', value: profile.category ? `${profile.category} (${profile.subCategory || 'General'})` : 'Not Set' },
  ...
  return (
    <div className="max-w-4xl mx-auto animate-in fade-in duration-500">
        ...
        <div className="px-8 pb-8">
          <div className="relative flex justify-between items-end -mt-12 mb-8">
            <div className="w-24 h-24 bg-white rounded-2xl p-1 shadow-lg overflow-hidden">
              <div className="w-full h-full bg-neutral-100 rounded-xl flex items-center justify-center text-neutral-400 font-bold text-3xl overflow-hidden">
                {profile.photoURL ? (
      ...
      {isAuthorizedAdmin && profile.role !== 'admin' && (
        <div className="mt-8 bg-emerald-50 rounded-3xl border border-emerald-100 p-8 shadow-sm animate-in zoom-in duration-500">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              <h3 className="text-xl font-black text-neutral-900 tracking-tight">Admin Access Available</h3>
              <p className="text-sm text-neutral-600 font-medium">Your email is authorized for Admin privileges.</p>
            </div>
          </div>
          <p className="text-neutral-600 text-sm mb-6 font-medium leading-relaxed">
            It looks like your account was registered with a Worker role. Since you are an authorized administrator, you can switch to the Admin role to access the control center and management tools.
          </p>
          <button
            onClick={handleSwitchToAdmin}
            disabled={loading}
            className="bg-neutral-900 text-white px-8 py-4 rounded-2xl font-black uppercase tracking-widest hover:bg-neutral-800 transition-all shadow-xl flex items-center gap-3 disabled:opacity-50"
          >
            {loading ? <Loader2 size={20} className="animate-spin" /> : <Zap size={20} className="text-emerald-400" />}
            Activate Admin Role
          </button>
        </div>
      )}
