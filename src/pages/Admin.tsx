    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-6">
          <div className="bg-neutral-900 p-8 rounded-[40px] text-white shadow-xl relative overflow-hidden h-full">
...
const FraudMonitoring: React.FC<{ claims: Claim[] }> = ({ claims }) => {
  ...
  const handleVPNCheck = () => {
    ...
    toast.promise(
      ...
      {
        loading: 'Scanning network for VPN/Proxy patterns...',
        success: () => {
          setIsScanning(false);
          return 'VPN Scan Complete: 2 suspicious connections flagged.';
        },
        error: 'Scan failed.',
      }
    );
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        ...
        <button 
          onClick={handleVPNCheck}
          disabled={isScanning}
          className="flex items-center gap-2 px-6 py-3 bg-red-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-red-700 transition-all shadow-lg disabled:opacity-50"
        >
          <ShieldAlert size={16} />
          {isScanning ? 'Scanning...' : 'Check for VPN Usage'}
        </button>
      </div>
