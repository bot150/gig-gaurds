    ...
    <div className="max-w-5xl mx-auto space-y-12 animate-in fade-in duration-700 pb-20">
      ...
      {!isAdmin && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-white p-6 rounded-[24px] border border-neutral-100 shadow-sm text-center group hover:border-emerald-500 transition-all">
            ...
            <h3 className="font-bold text-neutral-900 mb-1 text-sm">AI Chat Board</h3>
            <p className="text-xs text-neutral-500 mb-4">Talk to our AI assistant for instant answers.</p>
            <button 
              onClick={() => setShowChat(true)}
              className="w-full py-2.5 bg-neutral-900 text-white rounded-xl font-bold text-xs hover:bg-emerald-600 transition-colors"
            >
              Start Chat
            </button>
          </div>
          <div className="bg-white p-6 rounded-[24px] border border-neutral-100 shadow-sm text-center group hover:border-emerald-500 transition-all">
            ...
            <h3 className="font-bold text-neutral-900 mb-1 text-sm">Raise a Complaint</h3>
            <p className="text-xs text-neutral-500 mb-4">Submit a formal complaint to our admin team.</p>
            <button 
              onClick={() => setShowNewComplaint(true)}
              className="w-full py-2.5 bg-blue-600 text-white rounded-xl font-bold text-xs hover:bg-blue-700 transition-colors"
            >
              New Complaint
            </button>
          </div>
      ...
      <AnimatePresence>
        {showChat && (
          ...
          >
            <div className="max-w-2xl w-full relative">
              <AIChatBoard onClose={() => setShowChat(false)} />
            </div>
          </motion.div>
        )}
        ...
        {showNewComplaint && (
          <motion.div 
            ...
            className="fixed inset-0 bg-neutral-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto"
          >
            <div className="bg-white rounded-[32px] p-8 max-w-md w-full shadow-2xl relative my-8">
              <button onClick={() => setShowNewComplaint(false)} className="absolute top-6 right-6 text-neutral-400 hover:text-neutral-900 transition-colors">
                <X size={20} />
              </button>
              <h3 className="text-2xl font-black text-neutral-900 mb-4 tracking-tight">New Complaint</h3>
              <form onSubmit={handleSubmitComplaint} className="space-y-4">
                <div>
                  <label className="block text-xs font-bold text-neutral-500 uppercase tracking-widest mb-1.5 ml-1">Subject</label>
                  <input 
                    type="text" 
                    required
                    className="w-full px-5 py-3 bg-neutral-50 border border-neutral-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none text-sm"
                ...
                <div>
                  ...
                  <textarea 
                    required
                    rows={3}
                    className="w-full px-5 py-3 bg-neutral-50 border border-neutral-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none resize-none text-sm"
                ...
                <div>
                  <label className="block text-xs font-bold text-neutral-500 uppercase tracking-widest mb-1.5 ml-1">Payment ID (Optional)</label>
                  <input 
                    type="text" 
                    className="w-full px-5 py-3 bg-neutral-50 border border-neutral-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none text-sm"
                ...
                <div>
                  <label className="block text-xs font-bold text-neutral-500 uppercase tracking-widest mb-1.5 ml-1">Proof Image (Optional)</label>
                  <div className="relative">
                    ...
                    <label 
                      ...
                      className="flex items-center gap-3 p-3 bg-neutral-50 rounded-xl border border-dashed border-neutral-200 text-neutral-400 cursor-pointer hover:border-emerald-500 hover:text-emerald-500 transition-all"
                    >
                      <ImageIcon size={20} />
                ...
                <button 
                  type="submit"
                  className="w-full py-4 bg-neutral-900 text-white rounded-xl font-black uppercase tracking-widest text-xs hover:bg-emerald-600 transition-all shadow-lg hover:shadow-emerald-500/20 mt-2"
                >
                  Submit Complaint
                </button>
              </form>
